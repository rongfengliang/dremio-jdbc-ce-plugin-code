...
public final class JdbcRecordReader extends AbstractRecordReader {
   ...
   public void checkSchemaConsistency(List<JdbcToFieldMapping> mappings) {
      ...
      for(int i = 0; i < this.columns.size(); ++i) {
         String columnPath = ((SchemaPath)this.columns.get(i)).getAsUnescapedPath();
         if (!columnPath.toLowerCase(Locale.ROOT).equals(mappingsFieldNames.get(i))) {
            throw UserException.invalidMetadataError().addContext("Unexpected mismatch of column names, expected: " + columnPath + ", found: " + (String)mappingsFieldNames.get(i)).setAdditionalExceptionContext(new InvalidMetadataErrorContext(ImmutableList.of((List)this.columns.stream().map(BasePath::getAsUnescapedPath).collect(Collectors.toList()), mappingsFieldNames))).build(logger);
         }
      }
